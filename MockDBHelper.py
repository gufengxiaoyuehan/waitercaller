...
MOCK_REQUESTS = [{"_id":"1","table_number":"1","owner":"test@example.com","time":datetime.datetime.now()}]
...
class MockDBHelper:
    ...
    def add_request(self, number, owner):
        MOCK_REQUESTS.append(
            {"_id":str(number),"table_number":number,"owner":owner, "time":datetime.datetime.now()},
        )
        return number